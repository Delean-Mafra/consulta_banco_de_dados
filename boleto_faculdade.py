# Este script extrai informações de boletos de faculdade em PDF.
...
try:
    ...
    raise SystemExit("Instale a dependência: pip install pikepdf") from e
...
def find_data_vencimento(lines: List[str]) -> str | None:
    ...
    for i, l in enumerate(lines):
        if re.search(r'Vencimento\s*:', l, re.IGNORECASE):
            # olhar mesma linha e 2 seguintes
            window = lines[i:i+3]
            for w in window:
                dm = REGEX_DATA.search(w)
                if dm:
                    return dm.group(1)
    # fallback: procura por padrão de data próximo a palavra "Vencimento"
    text = "\n".join(lines)
...
def desbloquear_pdf(input_pdf: Path) -> None:
    """Desbloqueia um arquivo PDF protegido por senha."""
    try:
        with pikepdf.open(str(input_pdf), allow_overwriting_input=True) as pdf:
            pdf.save(str(input_pdf))
    except Exception as e:
        print(f"Aviso: Não foi possível desbloquear {input_pdf.name}: {e}")


def listar_pdfs_pasta(pasta: Path) -> List[Path]:
...
def desbloquear_pdfs_pasta(pasta: Path) -> None:
    """Desbloqueia todos os PDFs em uma pasta."""
    pdfs = listar_pdfs_pasta(pasta)
    if pdfs:
        print(f"Desbloqueando {len(pdfs)} PDF(s) na pasta...")
        for pdf_path in pdfs:
            try:
                desbloquear_pdf(pdf_path)
                print(f"  ✓ {pdf_path.name} desbloqueado")
            except Exception as e:
                print(f"  ✗ Erro ao desbloquear {pdf_path.name}: {e}")
...
def main() -> int:
    # Caminho padrão
    caminho_padrao = Path(r"C:\Users\Acer\Downloads")
    
    # Perguntar ao usuário pelo caminho
    print(f"Caminho padrão: {caminho_padrao}")
    entrada_usuario = input(f"Digite o caminho da pasta com os PDFs (Enter para usar o padrão): ").strip()
    
    if entrada_usuario:
        pasta_pdfs = Path(entrada_usuario)
    else:
        pasta_pdfs = caminho_padrao
    ...
    # Desbloquear todos os PDFs da pasta
    desbloquear_pdfs_pasta(pasta_pdfs)
    
    # Listar PDFs disponíveis
    pdfs = listar_pdfs_pasta(pasta_pdfs)
    
    if not pdfs:
        print(f"Nenhum arquivo PDF encontrado em '{pasta_pdfs}'")
        return 1
    
    # Se apenas 1 PDF, processar automaticamente
    if len(pdfs) == 1:
        pdf_selecionado = pdfs[0]
        print(f"\nArquivo encontrado: {pdf_selecionado.name}")
    else:
        # Se mais de 1 PDF, mostrar opções
        print(f"\n{len(pdfs)} arquivo(s) PDF encontrado(s) na pasta:")
        for idx, pdf in enumerate(pdfs, 1):
            print(f"  {idx}. {pdf.name}")
        
        while True:
            try:
                escolha = int(input(f"\nQual PDF você deseja processar? (1-{len(pdfs)}): "))
                if 1 <= escolha <= len(pdfs):
                    pdf_selecionado = pdfs[escolha - 1]
                    break
                else:
                    print(f"Número inválido. Digite um número entre 1 e {len(pdfs)}.")
            except ValueError:
    ...
    output_path = uploads_dir / (pdf_selecionado.stem + ".txt")
    
    # Processar o PDF
    print(f"\nProcessando: {pdf_selecionado.name}...")
    out = process_pdf(pdf_selecionado, output_path)
